...
def test_compute_phi_mathematical_safeguards_for_small_duration():
    # Large negative delta_time trying to push duration to negative/very small values
    result_neg_delta = compute_phi(revenue=10.0, cost=0.0, time_hours=1.0, delta_time=-1.5)
    # The multiplier is clamped to 0.1, making the duration at least 0.1 hours (6 mins).
    # Result should be exactly 10.0 / 0.1 = 100.0 rather than inf/error.
    assert result_neg_delta == 100.0

    # Very small explicit time_hours should be safely capped at 0.1 to prevent Φ -> ∞
    result_small_hours = compute_phi(revenue=10.0, cost=0.0, time_hours=0.00001)
    assert result_small_hours == 100.0
