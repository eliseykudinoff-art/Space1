...
class TestTask:
    ...
    def test_overdue_task(self):
        ...
        # Создаём задачу с deadline в будущем
        deadline = datetime.now() + timedelta(hours=1)
        task = create_task(title="Overdue", deadline=deadline)
        
        # Симулируем просрочку: меняем deadline на прошлое напрямую (тестовый сценарий)
        object.__setattr__(task, 'deadline', datetime.now() - timedelta(hours=1))
        task._update_deadline_fields()
