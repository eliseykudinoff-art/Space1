...
class MetricRegistry:
    ...
    def get(self, metric: str, default: Optional[float] = 0.0) -> float:
        """Get current EMA value for a metric. Creates metric if not found."""
    ...
    def update(self, metric: str, value: float, alpha: Optional[float] = None) -> float:
        """Update a metric. Alias for track()."""
        return self.track(metric, value, alpha=alpha)
