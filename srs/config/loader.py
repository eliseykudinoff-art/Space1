...
# Ищем config директорию: сначала в текущей рабочей директории, потом рядом с пакетом
import os as _os
_CWD_CONFIG = Path(_os.getcwd()) / "config"
_PKG_CONFIG = Path(__file__).parent.parent.parent.parent / "config"
CONFIG_DIR = _CWD_CONFIG if _CWD_CONFIG.exists() else _PKG_CONFIG
...
def load_constants(path: Optional[Path] = None) -> ConstantsConfig:
    ...
    if not path.exists():
        import warnings
        warnings.warn(f"Config file not found: {path}. Using default ConstantsConfig.", RuntimeWarning)
...
def load_weights(path: Optional[Path] = None) -> WeightsConfig:
    ...
    if not path.exists():
        import warnings
        warnings.warn(f"Config file not found: {path}. Using default WeightsConfig.", RuntimeWarning)
...
def load_rules(path: Optional[Path] = None) -> RulesConfig:
    ...
    if not path.exists():
        import warnings
        warnings.warn(f"Config file not found: {path}. Using default RulesConfig.", RuntimeWarning)
        return RulesConfig()
    data = _safe_load_config(path)
...
def load_prices(path: Optional[Path] = None) -> PricesConfig:
    ...
    if not path.exists():
        import warnings
        warnings.warn(f"Config file not found: {path}. Using default PricesConfig.", RuntimeWarning)
        return PricesConfig()
    data = _safe_load_config(path)
