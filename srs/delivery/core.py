    ...
    def send(self, task_id: str, result: Any, recipient: str, **kwargs) -> DeliveryResult:
        ...
        if success:
            self._delivered.append({
                "task_id": task_id, "result": result, "recipient": recipient,
                "timestamp": datetime.now().isoformat(), "metadata": kwargs
            })
            result_obj = DeliveryResult(
                success=True, status=DeliveryStatus.DELIVERED,
                message=f"Mock delivery to {recipient} succeeded",
                metadata={"recipient": recipient, "task_id": task_id}
            )
        else:
            result_obj = DeliveryResult(
                success=False, status=DeliveryStatus.FAILED,
...
class FileDeliveryAdapter(DeliveryAdapter):
    """File-based delivery adapter. Saves results to JSON files."""
    ...
    def send(self, task_id: str, result: Any, recipient: str, **kwargs) -> DeliveryResult:
        try:
            ...
            payload = {
                "task_id": task_id, "recipient": recipient, "result": result,
                "timestamp": datetime.now().isoformat(), "metadata": kwargs
            }
...
class DeliveryManager:
    ...
    def send(
        self, task_id: str, result: Any, recipient: str,
        adapter_name: Optional[str] = None, **kwargs
    ) -> DeliveryResult:
        name = adapter_name or self._default_adapter
        if not name:
            return DeliveryResult(success=False, status=DeliveryStatus.FAILED,
